    ...
    describe('single `context.addImportDeclaration` call', () => {
      describe('single non-type named import', () => {
        ...
        it('should add a single import statement', () => {
          const project = codegen(rootEntities, generators as Generators, defaultOptions)

          expect(
            project.getSourceFileOrThrow('./foo/gen-foo.ts').getImportDeclarations()
          ).toHaveLength(1)
        })
          ...
          expect(
            project
              ...
              .getImportDeclaration(
                (importDeclaration) =>
                  importDeclaration.getModuleSpecifierValue() === importSpecifier
              )
              ?.getText()
      ...
      describe('multiple type named imports', () => {
        describe('options.consolidateTypeImports: true', () => {
          ...
          it('should add a single import statement', () => {
            const project = codegen(rootEntities, generators as Generators, options)
          ...
          it('should be the correct import statement', () => {
            ...
            expect(
              project
                ...
                ?.getText()
            ).toMatchInlineSnapshot(`"import type { foo, bar, quz } from "foo-bar";"`)
          })
        })
          ...
          it('should be the correct import statement', () => {
            ...
            expect(
              project
                ...
                ?.getText()
            ).toMatchInlineSnapshot(`"import { type foo, type bar, type quz } from "foo-bar";"`)
          })
        })
      })
    })

...
import type { DeclarationEntity, Entity, ObjectPropertyEntity } from '@gents/parser'
...
const createNullishCoalescingExpression = (left: Expression, right: Expression) => {
...
const createIdentifierImport = (
  ...
  context.addImportDeclaration({
    specifier,
    named: options.named ? [{ name: identifier, typeOnly: Boolean(options?.typeOnly) }] : [],
  ...
  create: (_, context) =>
    factory.createCallExpression(
      createPropertyAccessChain([
        createIdentifierImport('faker', '@faker-js/faker', { named: true }, context),
        'datatype',
        'boolean'
      ]),
      undefined,
...
const isOneOfKindOrThrow = <TKind extends SyntaxKind>(
  ...
  if (!(kind as unknown[]).includes(node.kind)) {
    throw new TypeError(`Expected to handle node of syntax kind "${kind}", got "${node.kind}"`)
  }
  return node as TKind extends SyntaxKind.ExpressionStatement ? Expression : never
}
...
const identifierHints = [
  {
    ...
    create: (entity: ObjectPropertyEntity | DeclarationEntity) =>
      isOneOfCaseInsensitive(['company', 'organzation', 'merchant'], entity.name)
        ? 'company'
        : undefined
  },
  {
    ...
    create: (entity: DeclarationEntity | ObjectPropertyEntity) =>
      isOneOfCaseInsensitive(
        ['reviewer', 'human', 'person', 'user', 'profile', 'reviewers', 'employee'],
        entity.name
      )
        ? 'human'
        : undefined
        ...
        : isOneOfCaseInsensitive(['lastName', 'familyName'], entity.name)
          ...
          : isOneOfCaseInsensitive(['middleName'], entity.name)
            ...
            : entity.name === 'name' || entity.name === 'fullName'
              ? 'fullName'
              : context.hints.some((hint) => hint.name === 'company' && hint.level <= 1)
  ...
  },
  {
    name: 'currency',
    create: (entity: DeclarationEntity | ObjectPropertyEntity) =>
      ['currency', 'money'].includes(entity.name.toLowerCase()) ? 'currency' : undefined
  },
  {
    name: 'id',
    create: (entity: DeclarationEntity | ObjectPropertyEntity) =>
  ...
  },
  {
    ...
    create: (
      ...
      (context.hints.some((hint) => hint.name === 'currency' && hint.level <= 1) &&
...
export const generators: Generators = {
    ...
    create: (entity, context) => {
      if (!Array.isArray(entity.elements)) {
        return factory.createCallExpression(
          createPropertyAccessChain([{ name: 'faker' }, { name: 'helpers' }, { name: 'multiple' }]),
          undefined,
          [
            factory.createArrowFunction(
              ...
              context.next({ ...context, parentEntity: entity }, entity.elements) as Expression
            ),
                    ...
                    factory.createObjectLiteralExpression(
                      [
                        factory.createPropertyAssignment(
                          ...
                          factory.createCallExpression(
                            createPropertyAccessChain(['faker', 'number', 'int']),
                            undefined,
      ...
      return factory.createArrayLiteralExpression(
        Array.isArray(entity.elements)
          ...
          : (factory.createArrayLiteralExpression([]) as unknown as Expression[])
      )
    }
  },
  alias: {
    create: (entity) => {
      return factory.createCallExpression(
        factory.createIdentifier(camelcase(generateDeclarationName(entity.alias))),
        undefined,
        []
      )
    }
  ...
  reference: {
    create: (entity, context) => {
      if (entity.reference === 'Date') {
        return factory.createCallExpression(
          createPropertyAccessChain([{ name: 'faker' }, { name: 'date' }, { name: 'anytime' }]),
          undefined,
          []
        )
      }
      return factory.createCallExpression(
        factory.createIdentifier(camelcase(generateDeclarationName(entity.reference))),
        undefined,
        []
      )
    }
    ...
    create: (entity, context) => {
        ...
        factory.createTypeLiteralNode([
          factory.createPropertySignature(
            ...
            factory.createUnionTypeNode([
              ...
              factory.createArrayTypeNode(factory.createKeywordTypeNode(SyntaxKind.NumberKeyword))
            ])
          )
        ]),
        undefined
      ...
      const overloads =
        entity.declaration.type === 'union'
          ? canBeUndefined
            ? [
                // When union supports undefined and undefined is provided
                factory.createFunctionDeclaration(
                  [factory.createToken(SyntaxKind.ExportKeyword)],
                  undefined,
                  factory.createIdentifier(camelcase(generateDeclarationName(entity.name))),
                  [
                ...
                factory.createFunctionDeclaration(
                  ...
                  factory.createTypeReferenceNode(factory.createIdentifier(entity.name), undefined),
                  undefined
                ),
                // When _ is provided
                factory.createFunctionDeclaration(
                ...
                // When specific union member is provided
                factory.createFunctionDeclaration(
                  [factory.createToken(SyntaxKind.ExportKeyword)],
                  undefined,
                  factory.createIdentifier(camelcase(generateDeclarationName(entity.name))),
            ...
            : [
                ...
                // When union does not support undefined and undefined is provided
                factory.createFunctionDeclaration(
                  [factory.createToken(SyntaxKind.ExportKeyword)],
                  undefined,
                  factory.createIdentifier(camelcase(generateDeclarationName(entity.name))),
          ...
          : canBeUndefined
                  ...
                  [
                    factory.createTypeParameterDeclaration(
                      ...
                      factory.createUnionTypeNode([
                        ...
                        factory.createTypeQueryNode(factory.createIdentifier('_'), undefined)
                      ]),
                      undefined
                    )
                  ],
            ...
            : entity.declaration.type === 'object'
              ? [
                  // When no arguments are provided (user?: never)
                  factory.createFunctionDeclaration(
                    [factory.createToken(SyntaxKind.ExportKeyword)],
                    undefined,
                    factory.createIdentifier(camelcase(generateDeclarationName(entity.name))),
                  ...
                  // When partial entity is provided
                  factory.createFunctionDeclaration(
                    [factory.createToken(SyntaxKind.ExportKeyword)],
                    undefined,
                    factory.createIdentifier(camelcase(generateDeclarationName(entity.name))),
                    ...
                    factory.createTypeReferenceNode(
                      ...
                      [
                        ...
                        factory.createTypeLiteralNode([
                          factory.createPropertySignature(
                            undefined,
                            factory.createIdentifier('preferUndefinedSource'),
                            undefined,
      ...
      const mainFunction = factory.createFunctionDeclaration(
        [factory.createToken(SyntaxKind.ExportKeyword)],
        undefined,
        factory.createIdentifier(camelcase(generateDeclarationName(entity.name))),
        [
        ...
        undefined,
        factory.createBlock([
          // Seed faker if options.seed is provided
          ...
          entity.declaration.type === 'union'
            ? factory.createReturnStatement(
                factory.createCallExpression(
                  createIdentifierImport('merge', '@gents/gents', { named: true }, context),
                  undefined,
                  [
                    isOneOfKindOrThrow(
                      context.next(
                      ...
                      [SyntaxKind.ExpressionStatement, SyntaxKind.CallExpression]
                    ),
                    factory.createIdentifier(camelcase(entity.name)),
                    factory.createObjectLiteralExpression(
                      [
                        factory.createPropertyAssignment(
                          factory.createIdentifier('preferUndefinedSource'),
                          canBeUndefined ? factory.createTrue() : factory.createFalse()
                        )
                      ],
                      false
                    )
      ...
      return factory.createBlock([...(overloads ?? []), mainFunction])
    },
    hints: identifierHints
  },
  number: {
  ...
  string: {
    create: (entity, context) => {
      const nameHint = context.hints.find((hint) => hint.name === 'name' && hint.level <= 1)
      ...
      if (nameHint) {
        ...
        return factory.createCallExpression(
          createPropertyAccessChain([fakerImport, company ? 'company' : 'person', nameHint.value]),
          undefined,
          []
        )
      }
      ...
      const idHint = context.hints.find((hint) => hint.name === 'id' && hint.level === 1)
      if (idHint) {
        return factory.createCallExpression(
      ...
      const urlHint = context.hints.find((hint) => hint.name === 'url' && hint.level <= 1)
      if (urlHint) {
      ...
      const avatarHint = context.hints.find((hint) => hint.name === 'avatar' && hint.level <= 3)
      ...
      const unionMembersArray = factory.createArrayLiteralExpression(
        entity.values.map((value) => {
          ...
          const generatorFunction = factory.createArrowFunction(
            ...
            undefined,
            factory.createToken(SyntaxKind.EqualsGreaterThanToken),
            isOneOfKindOrThrow(context.next({ ...context, parentEntity: entity }, value), [
              SyntaxKind.CallExpression,
              SyntaxKind.Identifier,
              SyntaxKind.ArrayLiteralExpression,
              SyntaxKind.StringLiteral,
...
// Convert RuntimeSchema to TypeScript AST nodes
function runtimeSchemaToASTNode(schema: RuntimeSchema): Expression {
  switch (schema.type) {
    case 'object':
      return factory.createObjectLiteralExpression([
        factory.createPropertyAssignment(
        ...
        factory.createPropertyAssignment(
          ...
          factory.createObjectLiteralExpression(
            Object.entries(schema.properties).map(([key, propInfo]) =>
              factory.createPropertyAssignment(
                factory.createStringLiteral(key),
                factory.createObjectLiteralExpression([
                  factory.createPropertyAssignment(
                    factory.createIdentifier('schema'),
                    runtimeSchemaToASTNode(propInfo.schema)
                  ),
                  factory.createPropertyAssignment(
                    factory.createIdentifier('optional'),
                    propInfo.optional ? factory.createTrue() : factory.createFalse()
                  )
                ])
              )
            )
        ...
        factory.createPropertyAssignment(
          factory.createIdentifier('requiredProperties'),
          factory.createArrayLiteralExpression(
            schema.requiredProperties.map((prop) => factory.createStringLiteral(prop))
          )
        ),
        factory.createPropertyAssignment(
          factory.createIdentifier('optionalProperties'),
          factory.createArrayLiteralExpression(
            schema.optionalProperties.map((prop) => factory.createStringLiteral(prop))
          )
        )
      ])
    ...
    case 'array':
      if (schema.tuple) {
        return factory.createObjectLiteralExpression([
          ...
          factory.createPropertyAssignment(factory.createIdentifier('tuple'), factory.createTrue()),
          factory.createPropertyAssignment(
            factory.createIdentifier('elements'),
            factory.createArrayLiteralExpression(
              (schema.elements as RuntimeSchema[]).map((elem) => runtimeSchemaToASTNode(elem))
            )
          ),
          ...(schema.readonly
            ? [
        ...
        return factory.createObjectLiteralExpression([
          ...
          factory.createPropertyAssignment(
            factory.createIdentifier('elements'),
            runtimeSchemaToASTNode(schema.elements as RuntimeSchema)
          ),
          ...(schema.readonly
            ? [
                factory.createPropertyAssignment(
    ...
    case 'union':
      return factory.createObjectLiteralExpression([
        ...
        factory.createPropertyAssignment(
          factory.createIdentifier('members'),
          factory.createArrayLiteralExpression(
            schema.members.map((member) => runtimeSchemaToASTNode(member))
          )
        )
      ])
          ...
          schema.value === null
            ...
            : schema.value === undefined
              ...
              : typeof schema.value === 'string'
                ? factory.createStringLiteral(schema.value)
                : typeof schema.value === 'number'
                  ? factory.createNumericLiteral(schema.value)
                  : typeof schema.value === 'boolean'
                    ? schema.value
                      ? factory.createTrue()
                      : factory.createFalse()
                    : factory.createStringLiteral(String(schema.value))
        )
      ])
    ...
    case 'primitive':
      return factory.createObjectLiteralExpression([
        ...
        factory.createPropertyAssignment(
          factory.createIdentifier('primitiveType'),
          factory.createStringLiteral(schema.primitiveType)
        )
      ])
    ...
    case 'reference':
      return factory.createObjectLiteralExpression([
        ...
        factory.createPropertyAssignment(
          ...
          factory.createStringLiteral(schema.reference)
        )
      ])

    default:
      // Fallback to any primitive
      return factory.createObjectLiteralExpression([
        factory.createPropertyAssignment(
          factory.createIdentifier('type'),
          factory.createStringLiteral('primitive')
